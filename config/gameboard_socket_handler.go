...
func (h *GameboardSocketHandler) SetupGameboardHandlers(socket *socketio.Socket, authFunc func(socket *socketio.Socket, eventName string) (*models.User, error)) {
	...
	socket.On("dice:roll", func(event *socketio.EventPayload) {
		...
		log.Printf("DEBUG: Broadcasting dice roll to opponent for game %s, user %s, response %+v\n", gameID, user.ID, response)
	...
	socket.On("piece:move", func(event *socketio.EventPayload) {
		...
		// Support both legacy and new encrypted payloads for piece:move
		...
		if raw, ok := event.Data[0].(map[string]interface{}); ok {
			if encrypted, hasEncrypted := raw["user_data"]; hasEncrypted {
				...
				if jwtToken != "" && encStr != "" {
					...
					moveData = decrypted
					if _, ok := moveData["jwt_token"]; !ok && jwtToken != "" {
						moveData["jwt_token"] = jwtToken
					}
				} else {
					errorResp := models.ConnectionError{
						Status:    "error",
...
func (h *GameboardSocketHandler) broadcastDiceRollToOpponent(gameID, userID string, diceResponse *models.DiceRollResponse) {
	fmt.Printf("[DEBUG] broadcastDiceRollToOpponent called: gameID=%s, userID=%s\n", gameID, userID)
	// Get opponent user ID from the game
	opponentUserID := h.getOpponentUserID(gameID, userID)
	fmt.Printf("[DEBUG] Opponent userID resolved: %s\n", opponentUserID)
	if opponentUserID == "" {
	...
	fmt.Printf("[DEBUG] Sending dice update to opponent: %v\n", diceUpdate)
...
func (h *GameboardSocketHandler) sendMessageToUser(userID string, message map[string]interface{}, eventName string) {
	fmt.Printf("[DEBUG] sendMessageToUser called: userID=%s, eventName=%s\n", userID, eventName)
	if h.socketService.GetMessagingService() != nil {
		fmt.Printf("[DEBUG] Messaging service available, attempting to send\n")
		...
		if err != nil {
			fmt.Printf("[DEBUG] Messaging service failed: %v, falling back to direct socket emission\n", err)
			h.sendDirectToSockets(userID, eventName, message)
		} else {
			fmt.Printf("[DEBUG] Message sent successfully via messaging service\n")
		}
	} else {
		fmt.Printf("[DEBUG] No messaging service, using direct socket emission\n")
		h.sendDirectToSockets(userID, eventName, message)
	}
}
